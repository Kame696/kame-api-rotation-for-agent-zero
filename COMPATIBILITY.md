...
**KAME 1.8.1.3 — compatibility re-verified against Agent Zero v2.13 (2026-09-24): 15/15 symbols after re-pinning, 12/12 host facts, live harness green. The real-key sessions below were run on v2.12 with 1.8.1.2.**
...
| **V2.13** | **1.8.1.3** | **verified — current baseline** | `tools/a0_upgrade_check.py` on a fresh v2.13 checkout (Python 3.13): 14/15 symbols unchanged before re-pinning — the one change is `adaptive` (`unified_turn` hands `response_callback` a `response_preview`; KAME passes it through unread) — 12/12 host facts, live harness 78/78 green. No real-key session on v2.13 yet. |
| V2.12 | 1.8.1.2 | verified | `tools/a0_upgrade_check.py` all green on a fresh v2.12 checkout (symbols, host facts, live harness); `tools/live_a0_session.py` 12/12 real calls answered. |
| V2.12 | 1.8.1.1 | verified offline | 15/15 symbols unchanged and 12/12 host facts; never run in a real session. |
